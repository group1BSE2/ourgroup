...
public class File_Details {
    ...
    public static double numeric(String file, int sheet, String cellPosition) throws Exception {
        double projectName = 0.0;
        try {
            CellReference pname = new CellReference(cellPosition);
            FileInputStream fisi = new FileInputStream(new File(file));
            XSSFWorkbook workbook = new XSSFWorkbook(fisi);
            FormulaEvaluator evaluator = workbook.getCreationHelper().createFormulaEvaluator();
            ...
            if (row != null) {
                ...
                if (cell != null) {
                    switch (cell.getCellType()) {
                        case Cell.CELL_TYPE_NUMERIC:
                            projectName = cell.getNumericCellValue();

                            break;
                        case Cell.CELL_TYPE_STRING:
                            projectName = Double.valueOf(cell.getStringCellValue());
                            break;
                    }
                }
