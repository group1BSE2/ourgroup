...
public class Pick_GeneralNumeric {
    ...
    public static ArrayList<Double> pick(String file, int sheetNum, int cellNum) throws Exception {
        int rowNumber, x;
        double empty = 0;
        ...
        //x = sheetNum;// setting the sheet number...
        XSSFSheet spreadsheet = workbook.getSheetAt(sheetNum);
        String sname = workbook.getSheetName(sheetNum);

        Row row;
        Cell cell = null;
        for (rowNumber = 0; rowNumber < spreadsheet.getLastRowNum() + 1; ++rowNumber) {//To loop thru the rows in a sheet
            row = spreadsheet.getRow(rowNumber);
            cell = row.getCell(cellNum); //forks are in the eighth column...
            if (cell != null) {
    ...
    public static ArrayList<Double> pick_3(String file, int sheetNum, int cellNum, int rowNum) throws Exception {
        int j, x;
        double empty = 0;
        // array list to store the forks
        ArrayList<Double> lists = new ArrayList<>();
        ...
        Row row;
        Cell cell = null;
        for (j = rowNum; j < spreadsheet.getLastRowNum() + 1; ++j) {//To loop thru the rows in a sheet
